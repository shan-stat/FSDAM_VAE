...
# SH : opt_klParam added
fsdam <- function(dat, opt_numCode=ncol(dat), opt_seed=1, opt_model="n", opt_gpu=0, opt_k=100, opt_nEpochs=10000, opt_constr=c("newpenalization", "constrained", "none"), opt_tuneParam=10, opt_klParam=1, opt_penfun="mean", opt_ortho=1, opt_earlystop="no", verbose=FALSE) {    
...
#    reticulate::py_config() # to check 
    ...
    # SH : opt_klParam added; "kl_loss" added
    out=ae$fsdam$main(dat, as.integer(opt_numCode), as.integer(opt_seed), opt_model, as.integer(opt_gpu), as.integer(opt_k), as.integer(opt_nEpochs), opt_constr, opt_tuneParam, opt_klParam, opt_penfun, opt_ortho, opt_earlystop, verbose)    
    for (i in 1:length(out)) names(out[[i]])=c("reconstruct_loss", "kl_loss", "pen_loss", "y_pred", "code", "decoder_w", "decoder_b", "history")
    
    ret = list(
          data=dat
        , mse=sapply(1:opt_numCode, function (i) out[[i]]$reconstruct_loss)
        , kl.loss=sapply(1:opt_numCode, function (i) out[[i]]$kl_loss)
